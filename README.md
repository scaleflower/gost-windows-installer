...
```bash
curl -fsSL https://raw.githubusercontent.com/scaleflower/gost-windows-installer/main/install.sh | sudo bash
```
...
```bash
# 1. 下载脚本
curl -O https://raw.githubusercontent.com/scaleflower/gost-windows-installer/main/install.sh
...
1. 检查配置文件语法：`gost -C /etc/gost/config.json -D`（调试模式）
2. 查看服务状态：`sudo systemctl status gost`
3. 查看服务日志：`sudo journalctl -u gost -n 50`
